...
def collect_files(base_dir: Path, patterns: list[str]):
    """Yield all relevant files to include, except chatgpt_todo.txt (added last)."""
    for root, dirs, files in os.walk(base_dir):
        dirs[:] = [d for d in dirs if d not in EXCLUDE_DIRS]
        for file in files:
            file_path = Path(root) / file
            if file.lower() == TODO_FILE.lower():
                # skip, wird später am Ende angehängt
                continue
...
#
# This file is a combined snapshot of the entire Django project.
# It contains:
#   - All Python source files from all apps
#   - All Dockerfiles (any case, e.g. Dockerfile.mermaid)
#   - All .md and .txt documentation files
#   - The Makefile
#
# Purpose:
